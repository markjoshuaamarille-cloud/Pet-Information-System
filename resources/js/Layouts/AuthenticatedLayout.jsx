...
function CustomerAlertsModal({ type, alerts, appTimezone, onClose }) {
    ...
    const title = isAppointments ? 'Upcoming Appointments' : 'Health Monitoring (Due Soon)';
    const items = isAppointments ? alerts.upcomingAppointments : alerts.healthMonitoring;
                                ...
                                >
                                    ...
                                    {isAppointments && item.scheduled_at && (
                                        <p className="mt-2 text-xs opacity-80">
                                            {formatClinicDateTime(item.scheduled_at, appTimezone)}
                                        </p>
                                    )}
                                    ...
                                    {item.pet_id && (
                                        <Link
                                            href={route('pets.show', item.pet_id)}
...
const navItems = [
    ...
    { href: 'clients.index', label: 'Clients', roles: ['super_admin', 'receptionist'] },
    { href: 'pets.index', label: 'Pets', roles: ['super_admin', 'veterinarian', 'receptionist', 'customer', 'cashier', 'clinic_owner'], module: 'pets' },
    { href: 'appointments.index', label: 'Scheduling', roles: ['super_admin', 'veterinarian', 'receptionist', 'customer', 'clinic_owner'], module: 'scheduling' },
    { href: 'vaccinations.index', label: 'Vaccinations', roles: ['super_admin', 'veterinarian', 'receptionist', 'cashier', 'clinic_owner'], module: 'vaccinations' },
    { href: 'grooming.index', label: 'Grooming', roles: ['super_admin', 'groomer', 'receptionist', 'cashier', 'veterinarian', 'clinic_owner'], module: 'grooming' },
    { href: 'billing.index', label: 'Billing', roles: ['super_admin', 'cashier', 'receptionist', 'clinic_owner'], module: 'billing' },
    { href: 'pet-shop.index', label: 'Pet Shop', roles: ['super_admin', 'veterinarian', 'receptionist', 'customer', 'cashier', 'clinic_owner'], module: 'pet_shop' },
    { href: 'pet-shop-billing.index', label: 'Pet Shop Billing', roles: ['super_admin', 'cashier', 'receptionist', 'clinic_owner'], module: 'pet_shop_billing' },
    { href: 'pet-shop-reports.index', label: 'Shop Reports', roles: ['super_admin', 'clinic_owner', 'cashier', 'receptionist'], module: 'pet_shop' },
    { href: 'service-catalog.index', label: 'Service Catalog', roles: ['super_admin', 'veterinarian', 'receptionist', 'cashier', 'clinic_owner'], module: 'service_catalog' },
    { href: 'medicines.index', label: 'Inventory', roles: ['super_admin', 'veterinarian', 'receptionist', 'clinic_owner'], module: 'inventory' },
    { href: 'notifications.index', label: 'Notifications', roles: ['super_admin', 'veterinarian', 'receptionist', 'cashier', 'customer', 'clinic_owner'], module: 'notifications' },
...
function ClinicSwitcher({ activeClinic, assignedClinics, isPlatformAdmin }) {
    const switchClinic = (clinicId) => {
        router.post(route('clinic-context.store'), { clinic_id: clinicId || null }, { preserveScroll: true });
    };
...
export default function AuthenticatedLayout({ header, children }) {
    ...
    const upcomingAppointmentAlerts = customerAlerts?.upcomingAppointments ?? [];
            ...
            <nav className="border-b border-gray-100 bg-white">
                            ...
                            <div className="hidden space-x-4 sm:-my-px sm:ms-6 sm:flex sm:items-center">
                                {allowedNavItems.map((item) => (
                                    <NavLink
                                        ...
                                        active={route().current(item.href) || route().current(`${item.href.split('.')[0]}.*`)}
                                    >
                                        {item.label}
                                    </NavLink>
                                ))}
                                ...
                                <Dropdown>
                                    ...
                                    <Dropdown.Content width="56">
                                        {isCustomer && customerAlerts && (
                                            <>
                                                <Dropdown.Button onClick={() => openCustomerAlerts('appointments')}>
                                                ...
                                                <Dropdown.Button onClick={() => openCustomerAlerts('health')}>
                ...
                <div className={(showingNavigationDropdown ? 'block' : 'hidden') + ' sm:hidden'}>
                    <div className="space-y-1 pb-3 pt-2">
                        {isPlatformAdmin && assignedClinics.length > 0 && (
                            <div className="border-b border-gray-100 px-4 pb-3">
                                ...
                                <select
                                    ...
                                    value={activeClinic?.id ?? ''}
                                    onChange={(e) =>
                                        router.post(route('clinic-context.store'), {
                        ...
                        {allowedNavItems.map((item) => (
                            <ResponsiveNavLink key={item.href} href={route(item.href)}>
                                {item.label}
                            </ResponsiveNavLink>
                        ))}
                    </div>
