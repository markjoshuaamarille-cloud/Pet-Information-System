            ...
            <div className="py-8">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <form onSubmit={submit} className="mb-6 rounded-lg bg-white p-6 shadow">
                        <h3 className="mb-4 font-semibold">{editingId ? 'Edit Pet Record' : 'Register Pet'}</h3>
                        <div className="grid gap-4 sm:grid-cols-3">
                            ...
                            <div><InputLabel value="Birth Date" /><TextInput type="date" className="mt-1 block w-full" value={form.data.birth_date} onChange={(e) => form.setData('birth_date', e.target.value)} /></div>
                            <div><InputLabel value="Weight (kg)" /><TextInput type="number" step="0.01" min="0" className="mt-1 block w-full" value={form.data.weight} onChange={(e) => form.setData('weight', e.target.value)} /></div>
                            <div><InputLabel value="Color" /><TextInput className="mt-1 block w-full" value={form.data.color} onChange={(e) => form.setData('color', e.target.value)} /></div>
                            <div><InputLabel value="Microchip No" /><TextInput className="mt-1 block w-full" value={form.data.microchip_no} onChange={(e) => form.setData('microchip_no', e.target.value)} /></div>
                            <div>
                                <InputLabel value="Vaccination Status" />
                                <select className="mt-1 w-full rounded-md border-gray-300 shadow-sm" value={form.data.vaccination_status} onChange={(e) => form.setData('vaccination_status', e.target.value)}>
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {pets.map((p) => (
                                    <tr key={p.id}>
                                        ...
                                        <td className="px-4 py-3">{vaccinationStatusOptions.find((option) => option.value === p.vaccination_status)?.label ?? 'Unknown'}</td>
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            ...
                                            <button type="button" onClick={() => startEdit(p)} className="ms-3 text-amber-600 hover:underline">Edit</button>
